...
def crossoverPopulaton(population, scores, popSize, crossoverProbability, keep,crossovertype,selectionMethod):
    """
    ...
    for i in range(keep, popSize, 2):
        ...
        parent1, parent2 = pairSelection(population, scores, popSize,selectionMethod)
        ...
        if parentsCrossoverProbability < crossoverProbability:
            if(crossovertype==1):      
                ...
                offspring1, offspring2 = twopoint_crossover(crossoverLength, parent1, parent2)
...
def pairSelection(population, scores, popSize,selectionMethod):
    """
    ...
    if selectionMethod==1:
        ...
        parent1Id = tournament_selection(population,popSize,5)
        parent1 = population[parent1Id].copy()

        parent2Id = tournament_selection(population,popSize,5)
        parent2 = population[parent2Id].copy()
...
def tournament_selection(parent_population, population_size, tournament_size):
    ...
    for n in range(tournament_size):
        ...
        fitness_score = numpy.count_nonzero(parent == 1) / parent.shape[0]
...
def twopoint_crossover(individualLength, parent1, parent2):
    ...
    crossover_point2 = random.randint(crossover_point1, individualLength - 1)

    # The new offspring will have its first half of its genes taken from the first parent and second half of its genes taken from the second parent.
    offspring1 = numpy.concatenate(
        [parent1[0:crossover_point1], parent2[crossover_point1:crossover_point2], parent1[crossover_point2:]]
    )
    # The new offspring will have its first half of its genes taken from the second parent and second half of its genes taken from the first parent.
    offspring2 = numpy.concatenate(
        [parent2[0:crossover_point1], parent1[crossover_point1:crossover_point2], parent2[crossover_point2:]]
    )

    return offspring1, offspring2

def uniform_crossover(individualLenght,l,q):
    
    # converting the string to list for performing the crossover
        k = numpy.random.rand(individualLenght)
...
def GA(objf, lb, ub, dim, popSize, iters,_mp,crossovertype,selectionMethod):
    ...
    for l in range(iters):
        ...
        # crossover
        ga = crossoverPopulaton(ga, scores, popSize, cp, keep,crossovertype,selectionMethod)
