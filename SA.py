...
# SA Function
def simulated_annealing(min_values = [-5,-5], max_values = [5,5], mu = 0, sigma = 1, initial_temperature = 1.0, temperature_iterations = 1000, final_temperature = 0.0001, alpha = 0.99, target_function = target_function, verbose = True,isArithmetic=True):    
    ...
    i=0
    s=[]
    while(i<temperature_iterations):
    #while (Temperature > final_temperature): 
        ...
        if (fx_new < fx_best):
            ...
            best    = np.copy(guess)
        if isArithmetic:
            Temperature=Temperature-alpha       #arithmetic
        else :
            Temperature = alpha*Temperature     #geometric
        #print(Temperature)
    return s
